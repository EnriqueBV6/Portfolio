...
class UserProfile:
    ...
    Attributes:
        ...
        body_composition: Body composition analysis including body fat percentage and circumferences
    """
    ...
    def __post_init__(self) -> None:
        ...
        # NUEVA VALIDACIÓN: si hay body_composition, %grasa coherente con peso/altura
        if self.body_composition:
            pbf = self.body_composition.body_fat_percentage
            lbm = self.weight * (1 - pbf / 100)
            
            # Validación biológica
            min_lbm = 40 if self.sex.lower() == "male" else 28
            if lbm < min_lbm:
                raise ValueError(
                    f"Con {pbf}% grasa y {self.weight}kg peso, LBM sería {lbm}kg. "
                    f"Mínimo biológico es {min_lbm}kg. Verifica datos."
                )
